  ...
  "hasRecentWtcPayments" when {
    "there are no previous payments" should {
      ...
      "only call tax-credits-broker once when called multiple times" in {
        val connector = fakeTaxCreditsBrokerConnector(nino, Right(Seq.empty))
        ...
        await(service.hasRecentWtcPayments(nino)) shouldBe Right(false)
    ...
    "there are previous payments with positive amount less than 30 days ago" should {
      ...
      "continue to return true when called multiple times (not be broken by caching)" in {
        val metrics = FakeMobileHelpToSaveMetrics()
        val service = new TaxCreditsServiceImpl(logger, fakeTaxCreditsBrokerConnector(nino, Right(payments)), metrics, new FakeNinoWithoutWtcRepository(), fixedClock)

        await(service.hasRecentWtcPayments(nino)) shouldBe Right(true)
        metrics.taxCreditsBrokerCallCounter.getCount shouldBe 1
        metrics.taxCreditsCacheHitCounter.getCount shouldBe 0
        await(service.hasRecentWtcPayments(nino)) shouldBe Right(true)
    ...
    "there is an error getting previous payments" should {
      "return the error" in {
        val error = ErrorInfo.General
        val service = new TaxCreditsServiceImpl(logger, fakeTaxCreditsBrokerConnector(nino, Left(error)), FakeMobileHelpToSaveMetrics(), new FakeNinoWithoutWtcRepository(), fixedClock)
        await(service.hasRecentWtcPayments(nino)) shouldBe Left(error)
      }
    }
  }

  private def resultForPaymentsShouldBe(payments: Seq[Payment], expectedResult: Boolean): Unit = {
    val service = new TaxCreditsServiceImpl(logger, fakeTaxCreditsBrokerConnector(nino, Right(payments)), FakeMobileHelpToSaveMetrics(), new FakeNinoWithoutWtcRepository(), fixedClock)
    await(service.hasRecentWtcPayments(nino)) shouldBe Right(expectedResult)
  }

  private def fakeTaxCreditsBrokerConnector(expectedNino: Nino, previousPayments: Either[ErrorInfo, Seq[Payment]]) =
    new FakeTaxCreditsBrokerConnector(expectedNino, previousPayments)

  private class FakeTaxCreditsBrokerConnector(expectedNino: Nino, previousPayments: Either[ErrorInfo, Seq[Payment]]) extends TaxCreditsBrokerConnector {
    ...
    override def previousPayments(nino: Nino)(implicit hc: HeaderCarrier, ec: ExecutionContext): Future[Either[ErrorInfo, Seq[Payment]]] = {
      ...
      Future successful previousPayments
    }
  }
