...
class UserServiceSpec extends BaseSpec with EitherValues {
  ...
  private def fakeEligibilityRepo(eligibility: Option[Eligibility]) =
    new EligibilityRepo {
      ...
      override def setTestEligibility(eligibility: Eligibility, isHashed: Boolean): Future[Unit] = Future.successful(())
      override def getTestEligibilityRecord(nino: Nino): Future[Option[EligibilityRecord]] = Future.successful(None)
      override def getAllTestEligibilityRecords(): Future[Seq[EligibilityRecord]] = Future.successful(Seq.empty)
