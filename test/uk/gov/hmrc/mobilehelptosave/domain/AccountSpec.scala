...
class AccountSpec extends WordSpec with Matchers
  ...
  "apply" should {
    "omit nextPaymentMonthStartDate when payments will not be possible next month because it will be after the last bonus term" in {
      val thisMonthEndDate = helpToSaveAccount.bonusTerms.last.endDate
      val lastMonthHelpToSaveAccount = helpToSaveAccount.copy(thisMonthEndDate = thisMonthEndDate)

      val account = Account(lastMonthHelpToSaveAccount)
      account.nextPaymentMonthStartDate shouldBe None
      account shouldBe mobileHelpToSaveAccount.copy(thisMonthEndDate = thisMonthEndDate, nextPaymentMonthStartDate = None)
    }
  }
}
